...
`CPA_LICENSE_CLIENT_ID` 和商城签发的 Secret 是 `shop666`/`p.666ttt.net` 场景的启动前置条件。Secret 文件必须是单行、可读且权限为 `600`；脚本不会生成伪 Secret，也不会在输出中显示 Secret。
...
公开组合发布 tag 为 `v7.2.148-cpa.4`；`.env` 中至少确认以下组件镜像值与当前发布目录一致（组合 tag 不等于镜像 tag）：清单字段 `release_tag` 只用于定位公开 Release，不要写入 `CPA_IMAGE` 或 `CPAMP_IMAGE`。
