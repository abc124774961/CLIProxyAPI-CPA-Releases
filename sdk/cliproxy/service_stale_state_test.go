...
func TestServiceAuthReimportWithNewImportGenerationResetsRuntimeState(t *testing.T) {
	service := &Service{
		cfg:         &config.Config{},
		coreManager: coreauth.NewManager(nil, nil, nil),
	}
	authID := "service-reimport-generation-auth"
	t.Cleanup(func() { GlobalModelRegistry().UnregisterClient(authID) })
	...
	service.handleAuthUpdate(context.Background(), watcher.AuthUpdate{
		...
		Auth: &coreauth.Auth{
			...
			Provider: "codex",
			Status:   coreauth.StatusActive,
			Metadata: map[string]any{"cpamp_import": map[string]any{"imported_at": "2026-08-30T11:00:00Z"}},
		},
	})
	stored, ok := service.coreManager.GetByID(authID)
	if !ok || stored == nil {
		t.Fatal("initial auth was not registered")
	}
	stored.Quota = coreauth.QuotaState{Exceeded: true, Reason: "usage_limit_reached"}
	stored.ModelStates = map[string]*coreauth.ModelState{"gpt-5": {Unavailable: true}}
	service.coreManager.Update(context.Background(), stored)
	...
	service.handleAuthUpdate(context.Background(), watcher.AuthUpdate{
		...
		Auth: &coreauth.Auth{
			...
			Provider: "codex",
			Status:   coreauth.StatusActive,
			Metadata: map[string]any{"cpamp_import": map[string]any{"imported_at": "2026-08-30T12:00:00Z"}},
		},
	})
	updated, ok := service.coreManager.GetByID(authID)
	if !ok || updated == nil {
		t.Fatal("re-imported auth was not registered")
	}
	if updated.Quota.Exceeded || len(updated.ModelStates) != 0 {
		t.Fatalf("re-imported auth retained runtime state: quota=%#v models=%#v", updated.Quota, updated.ModelStates)
	}
}
