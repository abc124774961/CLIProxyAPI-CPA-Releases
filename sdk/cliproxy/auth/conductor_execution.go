...
func (m *Manager) Execute(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	ctx = m.withCodexQuotaOverdraftTracking(ctx, normalized)
...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	ctx = withErrorHandlingStart(ctx)
	req, opts = cliproxysession.Enrich(req, opts)
	if m.HomeEnabled() {
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		if authErr != nil {
			...
			lastErr = authErr
			if temporaryErr != nil {
				shouldSwitch, errWait := m.transientRetryAction(ctx, temporaryErr, &temporarySwitches)
				if errWait != nil {
					releaseRuntimeSlot(releaseRuntime)
					return cliproxyexecutor.Response{}, errWait
				}
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		if errStream != nil {
			...
			if rerr := resultErrorFromError(errStream); isTemporaryFailureResult(rerr) {
				shouldSwitch, errWait := m.transientRetryAction(ctx, rerr, &temporarySwitches)
				if errWait != nil {
					return nil, errWait
				}
				if !shouldSwitch {
