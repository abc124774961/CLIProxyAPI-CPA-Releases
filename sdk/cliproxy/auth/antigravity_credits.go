...
// ClearAntigravityCreditsHint removes the cached credits hint for an auth.
// Deleting a credential must invalidate this identity-scoped state so a later
// import cannot inherit the previous account's availability decision.
func ClearAntigravityCreditsHint(ctx context.Context, authID string) error {
	authID = strings.TrimSpace(authID)
	if authID == "" {
		return nil
	}
	...
	if homeMode {
		...
		_, errDel := client.KVDel(ctx, antigravityCreditsHintKey(authID))
		return errDel
	}
