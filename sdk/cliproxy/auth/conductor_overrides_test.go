...
func TestManager_CodexUnknownProviderForModel_FallsBackToNextTeamAuth(t *testing.T) {
	m := NewManager(nil, nil, nil)
	executor := &authFallbackExecutor{
		id: "codex",
		executeErrors: map[string]error{
			"aa-codex-team": &Error{
				...
				HTTPStatus: http.StatusBadRequest,
				Message:    "unknown provider for model gpt-5.5",
			},
		},
	}
	m.RegisterExecutor(executor)
	...
	if state == nil || !state.Unavailable {
		t.Fatalf("expected current model to be cooled on bad auth, got %#v", state)
	}
}
