...
func (m *Manager) setConfigSnapshotLocked(cfg *internalconfig.Config) bool {
	...
	m.refreshAPIKeyGroupPolicies(cfg)
...
func normalizedQuotaResetReason(value string) string {
	value = strings.ToLower(strings.TrimSpace(value))
	return strings.NewReplacer("-", "_", " ", "_").Replace(value)
}

func quotaResetRuntimeReason(value string) bool {
	switch normalizedQuotaResetReason(value) {
	case runtimeSkipReasonQuotaPreempt, runtimeSkipReasonUsageLimitReached, "codex_usage_limit_reached":
		return true
	default:
		return false
	}
...
func quotaResetStateReason(value string) bool {
	value = normalizedQuotaResetReason(value)
	if value == "" {
		return false
	}
	return quotaResetRuntimeReason(value) || strings.Contains(value, "quota") || strings.Contains(value, "usage_limit")
}

// shouldRecoverDisabledQuotaAuth distinguishes quota-driven disables from
// operator disables and terminal credential invalidation. Only the former may
// be re-enabled by a successful quota reset.
func shouldRecoverDisabledQuotaAuth(auth *Auth, runtime RuntimeLimitSnapshot) bool {
	if auth == nil || (!auth.Disabled && auth.Status != StatusDisabled) {
		return false
	}
	if strings.EqualFold(strings.TrimSpace(auth.StatusMessage), "credential invalidated") || isTerminalCredentialResultError(auth.LastError) {
		return false
	}
	if quotaResetRuntimeReason(runtime.LastSkipReason) || quotaResetStateReason(auth.Quota.Reason) {
		return true
	}
	for _, state := range auth.ModelStates {
		if state != nil && quotaResetStateReason(state.Quota.Reason) {
			return true
		}
	}
	// Older CPA files may retain only disabled=true after a process restart;
	// runtime skip metadata is in-memory and therefore absent from those files.
	// An explicit quota reset is the recovery signal for such Codex entries.
	if strings.EqualFold(strings.TrimSpace(executorKeyFromAuth(auth)), "codex") && strings.TrimSpace(auth.StatusMessage) == "" {
		return true
	}
	return false
}
...
func (m *Manager) ResetQuota(ctx context.Context, authID string) (*Auth, []string, error) {
	...
	runtimeSnapshot := auth.RuntimeLimitSnapshot(now)
	canRecoverDisabled := runtimeSnapshot.CurrentConcurrency == 0 &&
		(auth.hasQuotaResetRuntimeState(now) || shouldRecoverDisabledQuotaAuth(auth, runtimeSnapshot))
	auth.resetQuotaRuntimeState()
	if canRecoverDisabled {
		auth.Disabled = false
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	cooldownStateChanged := false
	skipSynchronousPersist := false

	m.mu.Lock()
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		now := time.Now()
		if !result.Success && isTemporaryFailureResult(result.Error) {
			m.errorHandlingMetrics.recordTemporaryFailure(temporaryFailureKind(result.Error))
		}
		...
		skipSynchronousPersist = !result.Success && m.keepsTransientAccountActive(result.Error)
		...
		if terminalCredential {
			...
			if modelKey != "" {
				if !m.shouldSkipCredentialCooldown(result.Error) {
				...
				disableCooling := m.cooldownDisabledForAuth(auth)
				keepTransientActive := m.keepsTransientAccountActive(result.Error)
				applyAuthFailureStateWithTransientPolicy(auth, result.Error, result.RetryAfter, now, disableCooling, keepTransientActive)
				// Temporary edge/network failures are request-local evidence. Avoid
				// synchronously writing the credential file/database on this hot error
				// path; hooks and the in-memory scheduler still observe the result.
				skipSynchronousPersist = skipSynchronousPersist || keepTransientActive
			}
		}
		...
		// A classified temporary network/edge failure is request-local evidence.
		// When the runtime policy keeps the credential active, do not also apply
		// the generic in-memory selection freeze: that would silently pause the
		// account despite skipping persistent cooldown state. Auth, quota, model,
		// and other non-temporary failures retain the existing runtime freeze.
		if !m.keepsTransientAccountActive(result.Error) {
	...
	// Injected requests provide direct business evidence. Ordinary requests use
	// the separate bounded verifier only for definite subscription-quota errors.
	if CodexQuotaOverdraftWasInjected(ctx, result.AuthID) {
		if result.Success {
			m.ObserveCodexQuotaOverdraftBusinessSuccess(ctx, result)
		} else {
			m.ObserveCodexQuotaOverdraftBusinessQuotaFailure(ctx, result)
		}
	} else {
		m.HandleCodexQuotaResult(ctx, result)
	}
...
func cloneError(err *Error) *Error {
	...
	setErrorFailureKind(out, errorFailureKind(err))
	if evidence := errorEvidence(err); evidence != nil {
		setErrorEvidence(out, evidence.clone())
	}
	return out
}
...
func resultErrorFromError(err error) *Error {
	...
	resultErr = enrichErrorEvidence(resultErr, err)
	switch {
...
// shouldSkipCredentialCooldown applies both the legacy request/lifecycle
// exemptions and the runtime temporary-error policy. Temporary edge,
// capacity, transport, and stream failures do not create persistent account
// cooldown when the corresponding setting is enabled.
func (m *Manager) shouldSkipCredentialCooldown(err *Error) bool {
	if shouldSkipCredentialCooldown(err) {
		return true
	}
	return m.keepsTransientAccountActive(err)
}
...
func isRequestInvalidError(err error) bool {
	...
	// Keep the request-fault decision aligned with the bounded classifier used
	// for runtime retry decisions. Some upstreams return a plain-text
	// "response item not found" or similar parameter error that does not carry
	// one of clienterror's structured codes. Rate-limit and payment statuses
	// remain authoritative, and an authentication body must still follow the
	// credential path.
	if status == http.StatusPaymentRequired || status == http.StatusTooManyRequests {
		return false
	}
	lower := strings.ToLower(err.Error())
	if status == http.StatusUnauthorized && strings.Contains(lower, "authentication_error") {
		return false
	}
	return clientRequestBody(errorTextForRequestClassification(err))
}

// errorTextForRequestClassification combines the ordinary error message with
// a bounded response body when an upstream error exposes one separately. A
// number of HTTP adapters use a generic message such as "upstream request
// failed" while keeping the actionable request code in ResponseBody; the
// request-fault decision must inspect both so those failures do not rotate a
// healthy credential.
func errorTextForRequestClassification(err error) string {
	if err == nil {
		return ""
	}
	message := err.Error()
...
// applyAuthFailureStateWithTransientPolicy is the account-level counterpart
// to Manager.shouldSkipCredentialCooldown. It preserves the established
// authentication/quota handling while leaving classified temporary failures
// active when configured.
func applyAuthFailureStateWithTransientPolicy(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time, disableCooling bool, keepTransientActive bool) {
	if auth == nil || shouldSkipCredentialCooldown(resultErr) {
		return
	}
	if keepTransientActive && isTemporaryFailureKind(temporaryFailureKind(resultErr)) {
		return
	}
	applyAuthFailureState(auth, resultErr, retryAfter, now, disableCooling)
}
