...
func (m *Manager) preparedExecutionModelsWithAliasForTailBurst(auth *Auth, routeModel string, tailBurst bool) ([]string, bool, OAuthModelAliasResult, *apiKeyModelRoutingSnapshot) {
	candidates, pooled, aliasResult, routing := m.executionModelCandidatesWithAlias(auth, routeModel)
	return m.filterExecutionModelsWithTailBurst(auth, routeModel, candidates, pooled, tailBurst), pooled, aliasResult, routing
}
