...
// ResponseHeaders exposes the initial upstream headers to the failure
// classifier without making stream bootstrap errors part of the public API.
func (e *streamBootstrapError) ResponseHeaders() http.Header {
	return e.Headers()
}
...
func (m *Manager) wrapStreamResult(ctx context.Context, auth *Auth, provider, affinityModel, resultModel string, metadata map[string]any, headers http.Header, buffered []cliproxyexecutor.StreamChunk, remaining <-chan cliproxyexecutor.StreamChunk, aliasResult OAuthModelAliasResult, ephemeralResult bool) *cliproxyexecutor.StreamResult {
	out := make(chan cliproxyexecutor.StreamChunk)
	go func() {
		defer close(out)
		var failed bool
		outputStarted := false
		outputStartRecorded := false
		forward := true
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				...
				if rerr != nil {
					evidence := errorEvidence(rerr)
					if evidence == nil {
						evidence = &FailureEvidence{}
					}
					evidence.OutputStarted = outputStarted
					setErrorEvidence(rerr, evidence)
					classifyFailure(rerr, nil)
				}
			...
			if !outputStartRecorded {
				outputStartRecorded = true
				m.errorHandlingMetrics.firstOutputCount.Add(1)
				if started := errorHandlingStart(ctx); !started.IsZero() {
					m.errorHandlingMetrics.firstOutputDelayMilliseconds.Add(uint64(time.Since(started).Milliseconds()))
				}
			}
			outputStarted = true
		...
		if !failed && (ephemeralResult || claudeOAuthRequestCancellation(ctx, auth, nil) == nil) {
			m.confirmCacheAffinityBinding(provider, affinityModel, auth.ID, metadata)
...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult, routing *apiKeyModelRoutingSnapshot, allowRetry bool, ephemeralResult bool, unauthorizedRefreshTried map[string]struct{}) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		if errStream != nil {
			...
			lastErr = errStream
			if isTemporaryFailureResult(resultErrorFromError(errStream)) {
				break
			}
			continue
		}
		...
		if bootstrapErr != nil {
			...
			if isTemporaryFailureResult(resultErrorFromError(bootstrapErr)) {
				rerr := resultErrorFromError(bootstrapErr)
				result := Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: false, Error: rerr}
				result.RetryAfter = retryAfterFromError(bootstrapErr)
				m.recordExecutionResult(ctx, result, auth, ephemeralResult)
		...
		return m.wrapStreamResult(ctx, auth.Clone(), provider, routeModel, resultModel, execOpts.Metadata, streamResult.Headers, buffered, remaining, attemptAliasResult, ephemeralResult), nil
	}
	if lastErr == nil {
