...
func (m *Manager) refreshAuthForRequest(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	...
	// A watcher update may install a provider runtime while Refresh is in
...
func isTerminalCredentialFailure(err error) bool {
	...
	var terminalErr terminalCredentialFailure
	if errors.As(err, &terminalErr) && terminalErr != nil && terminalErr.TerminalCredentialFailure() {
		return true
	}
