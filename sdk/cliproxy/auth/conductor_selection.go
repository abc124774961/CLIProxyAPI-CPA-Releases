...
// ListRuntimeViews returns lightweight runtime projections without cloning credential metadata.
func (m *Manager) ListRuntimeViews() []RuntimeAuthView {
	...
	for _, auth := range m.auths {
		...
		views = append(views, RuntimeAuthView{ID: auth.ID, FileName: auth.FileName, Runtime: auth.Runtime})
	}
	return views
}
