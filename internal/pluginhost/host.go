...
func (h *Host) ApplyConfig(ctx context.Context, cfg *config.Config) {
	...
	for _, file := range files {
		...
		if errLicense := h.pluginIDLicenseError(file.ID); errLicense != nil {
			h.mu.Lock()
			loaded := h.loaded[file.ID]
			h.mu.Unlock()
			if loaded != nil {
				h.discardUnlicensedPlugin(file.ID, loaded)
			}
			log.WithFields(pluginLogFields(file.ID, "", file.Version, file.Path)).Warnf("pluginhost: plugin withheld by license gate before load: %v", errLicense)
			continue
		}
		if errIntegrity := h.pluginFileIntegrityError(file); errIntegrity != nil {
			h.mu.Lock()
			loaded := h.loaded[file.ID]
			h.mu.Unlock()
			if loaded != nil {
			...
			log.WithFields(pluginLogFields(file.ID, "", file.Version, file.Path)).Warnf("pluginhost: plugin withheld by integrity gate before load: %v", errIntegrity)
			continue
		}
		h.mu.Lock()
		lp := h.loaded[file.ID]
		...
		if errLicense := h.pluginLicenseError(plugin.Metadata); errLicense != nil {
			log.WithFields(pluginLogFieldsFromMetadata(file.ID, plugin.Metadata, file.Path)).Warnf("pluginhost: plugin withheld by license gate: %v", errLicense)
			h.discardUnlicensedPlugin(file.ID, lp)
			continue
		}
		h.mu.Lock()
		if lp != nil {
