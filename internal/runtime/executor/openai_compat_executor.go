...
func (e *OpenAICompatExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		err = newStatusErrFromResponse(httpResp, b)
		return resp, err
	}
...
func (e *OpenAICompatExecutor) executeImages(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, endpointPath string) (resp cliproxyexecutor.Response, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		err = newStatusErrFromResponse(httpResp, body)
		return resp, err
	}
...
func (e *OpenAICompatExecutor) executeImagesStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, endpointPath string) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		helps.AppendAPIResponseChunk(ctx, e.cfg, body)
		helps.LogWithRequestID(ctx).Debugf("request error, error status: %d, error message: %s", httpResp.StatusCode, helps.SummarizeErrorBody(httpResp.Header.Get("Content-Type"), body))
		return nil, newStatusErrFromResponse(httpResp, body)
	}
...
const maxStatusErrorBodyBytes = 8 * 1024

func newStatusErrFromResponse(resp *http.Response, body []byte) statusErr {
	err := statusErr{}
	if resp != nil {
	...
	if len(body) > maxStatusErrorBodyBytes {
		err.body = append([]byte(nil), body[:maxStatusErrorBodyBytes]...)
	} else {
		err.body = append([]byte(nil), body...)
	}
	err.msg = string(err.body)
	return err
}
...
func (e statusErr) ResponseHeaders() http.Header {
	if e.headers == nil {
		return nil
	}
