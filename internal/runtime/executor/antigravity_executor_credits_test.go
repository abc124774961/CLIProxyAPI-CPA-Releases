...
func TestClearAntigravityAuthStateRemovesLocalCaches(t *testing.T) {
	resetAntigravityCreditsRetryState()
	t.Cleanup(resetAntigravityCreditsRetryState)
	const authID = "remove-antigravity-state"
	antigravityCreditsFailureByAuth.Store(authID, antigravityCreditsFailureState{PermanentlyDisabled: true})
	antigravityCreditsBalanceByAuth.Store(authID, antigravityCreditsBalance{Known: true})
	antigravityCreditsHintRefreshByID.Store(authID, &antigravityCreditsHintRefreshState{})
	antigravityShortCooldownByAuth.Store(authID+"|claude-sonnet-4-6|sc", time.Now().Add(time.Minute))
	cliproxyauth.SetAntigravityCreditsHint(authID, cliproxyauth.AntigravityCreditsHint{Known: true, Available: false})

	ClearAntigravityAuthState(context.Background(), authID)

	for name, cache := range map[string]*sync.Map{
		"failure": &antigravityCreditsFailureByAuth,
		"balance": &antigravityCreditsBalanceByAuth,
		"refresh": &antigravityCreditsHintRefreshByID,
	} {
		if _, ok := cache.Load(authID); ok {
			t.Fatalf("%s cache still contains auth", name)
		}
	}
	antigravityShortCooldownByAuth.Range(func(key, _ any) bool {
		if strings.HasPrefix(key.(string), authID+"|") {
			t.Fatalf("cooldown cache still contains auth")
		}
		return true
	})
	if _, ok := cliproxyauth.GetAntigravityCreditsHint(authID); ok {
		t.Fatal("credits hint cache still contains auth")
	}
}
