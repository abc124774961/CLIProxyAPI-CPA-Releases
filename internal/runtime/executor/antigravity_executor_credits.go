...
// ClearAntigravityAuthState drops all executor-side state keyed by one auth.
// This is called after credential deletion so a re-imported account starts
// with a clean credits decision and no short cooldown inherited from its
// previous runtime generation.
func ClearAntigravityAuthState(ctx context.Context, authID string, models ...string) {
	...
	antigravityShortCooldownByAuth.Range(func(key, _ any) bool {
		if keyString, ok := key.(string); ok && strings.HasPrefix(keyString, authID+"|") {
			antigravityShortCooldownByAuth.Delete(key)
		}
		return true
	})
	if client, homeMode, errClient := currentAntigravityKVClient(); homeMode && errClient == nil && client != nil {
		if ctx == nil {
			ctx = context.Background()
		}
		keys := []string{
			antigravityCreditsBalanceKey(authID),
			antigravityCreditsRefreshLockKey(authID),
		}
		for _, model := range models {
			if key := antigravityShortCooldownKVKey(&cliproxyauth.Auth{ID: authID}, model); key != "" {
				keys = append(keys, key)
			}
		}
		_, _ = client.KVDel(ctx, keys...)
	}
	_ = cliproxyauth.ClearAntigravityCreditsHint(ctx, authID)
}
