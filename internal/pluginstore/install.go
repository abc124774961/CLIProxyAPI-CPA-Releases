...
type InstallOptions struct {
	...
	// RejectLoaded prevents replacement of a library that is currently loaded.
	// It is used by encrypted premium-plugin installs where replacing bytes at
	// the same version would leave the running process on an unverified binary.
	RejectLoaded bool
...
// InstallLibrary installs an already verified dynamic library into the same
// versioned layout used by normal plugin-store archives. Callers that obtain
// the bytes from an encrypted package should verify/decrypt them before
// invoking this function.
func InstallLibrary(libraryData []byte, id, version string, options InstallOptions) (InstallResult, error) {
	return installLibrary(libraryData, 0o755, id, version, options)
}

func installLibrary(libraryData []byte, mode os.FileMode, id, version string, options InstallOptions) (InstallResult, error) {
	options = normalizeInstallOptions(options)
	id = strings.TrimSpace(id)
	version = normalizeVersion(version)
	if !validPluginID(id) {
	...
	if len(libraryData) == 0 {
		return InstallResult{}, errors.New("plugin library is empty")
	}
	targetPath, errTarget := installTargetPath(options, id, version)
	if errTarget != nil {
		return InstallResult{}, errTarget
	...
	return InstallResult{ID: id, Version: version, Path: targetPath, Overwritten: overwritten}, nil
}
...
// InstalledPluginVersion describes one versioned dynamic library present in a
// plugin directory. Unlike DiscoverPluginFiles, which intentionally selects a
// single file per plugin for runtime loading, this helper returns every
// version so management endpoints can offer upgrade and rollback safely.
...
// DiscoverInstalledPluginVersions lists all versioned libraries for id on the
// requested platform. The result is limited to the normal versioned layout
// and never follows paths outside the supplied plugin directory.
func DiscoverInstalledPluginVersions(root, id, goos, goarch string) ([]InstalledPluginVersion, error) {
	...
	extension := pluginExtension(goos)
	seen := make(map[string]struct{})
	versions := make([]InstalledPluginVersion, 0)
	for _, dir := range []string{filepath.Join(root, goos, goarch), root} {
		...
		for _, entry := range entries {
			if entry == nil || !entry.Type().IsRegular() || !strings.HasSuffix(strings.ToLower(entry.Name()), extension) {
				continue
			}
			file, okFile := pluginFileInfoFromPath(filepath.Join(dir, entry.Name()), extension)
			if !okFile || file.ID != id || strings.TrimSpace(file.Version) == "" {
				continue
			}
			...
			versions = append(versions, InstalledPluginVersion{ID: file.ID, Version: file.Version, Path: cleanPath})
		}
	}
	sort.SliceStable(versions, func(i, j int) bool {
		return compareInstalledVersions(versions[i].Version, versions[j].Version) > 0
	})
	return versions, nil
}

func compareInstalledVersions(a, b string) int {
	a = normalizeVersion(a)
	b = normalizeVersion(b)
	if a == b {
		return 0
	}
	if validPluginVersion(a) && validPluginVersion(b) {
		...
		for i := 0; i < length; i++ {
			av, aok := installedVersionSegment(as, i)
			bv, bok := installedVersionSegment(bs, i)
			if aok && bok && av != bv {
				if av > bv {
					return 1
				}
...
func installedVersionSegment(segments []string, index int) (int64, bool) {
	if index >= len(segments) {
		return 0, true
	}
	number, err := strconv.ParseInt(segments[index], 10, 64)
	if err != nil || number < 0 {
		return 0, false
	}
	return number, true
...
func loadedPluginInstallBlocked(options InstallOptions) bool {
	...
	return options.RejectLoaded || strings.EqualFold(options.GOOS, "windows")
}
