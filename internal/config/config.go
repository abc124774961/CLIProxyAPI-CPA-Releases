...
type Config struct {
	...
	// License configures CPA storefront authorization.
	...
	// ErrorHandling contains the built-in policy for classified temporary
	// upstream/network failures. It is intentionally not loaded from or written
	// to YAML; the runtime always uses DefaultErrorHandlingConfig(). The field is
	// retained in the in-memory model so older embedders can compile unchanged.
	ErrorHandling ErrorHandlingConfig `yaml:"-" json:"-"`
