...
type CodexConfig struct {
	...
	// Overdraft enables bounded quota verification for eligible Team OAuth credentials.
	Overdraft CodexOverdraftConfig `yaml:"overdraft" json:"overdraft"`
}

// CodexOverdraftConfig controls account-level Codex quota overdraft probing.
// The feature is opt-in because each probe consumes a real upstream request.
...
type CodexCacheAffinityConfig struct {
	...
	PrefixHeatMaxEntries      int     `yaml:"prefix-heat-max-entries" json:"prefix-heat-max-entries"`
	PrefixHeatMinBytes        int     `yaml:"prefix-heat-min-bytes" json:"prefix-heat-min-bytes"`
...
// CodexTailBurstToolInjectionConfig controls the internal tool declaration for tail-burst requests.
...
type RoutingConfig struct {
	...
	// Supported values: "concurrency-balanced" (default), "round-robin", "weighted-round-robin", "fill-first".
