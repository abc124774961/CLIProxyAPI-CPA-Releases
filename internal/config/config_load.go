...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	if err != nil {
		if optional {
			if os.IsNotExist(err) || errors.Is(err, syscall.EISDIR) {
				...
				cfg := &Config{CredentialInFlight: DefaultCredentialInFlightConfig(), ErrorHandling: DefaultErrorHandlingConfig(), License: DefaultLicenseConfig()}
	...
	if errValidate := validateCredentialWeightYAML(data); errValidate != nil {
		if optional {
			cfgOptional := &Config{CredentialInFlight: DefaultCredentialInFlightConfig(), ErrorHandling: DefaultErrorHandlingConfig(), License: DefaultLicenseConfig()}
	...
	// yaml:"-", so legacy error-handling blocks are ignored on load.
