...
// claudeOAuthSessionCaches keys one session cache per effective egress tuple.
//
...
func claudeOAuthSessionCache(proxyURL string) tls.ClientSessionCache {
	return claudeOAuthSessionCacheForEgress(proxyURL, "")
}

func claudeOAuthSessionCacheForEgress(proxyURL string, sourceIP string) tls.ClientSessionCache {
	cacheKey := strings.TrimSpace(proxyURL) + "\x00" + strings.TrimSpace(sourceIP)
	return claudeOAuthSessionCaches.GetOrAdd(cacheKey, func() tls.ClientSessionCache {
...
func newUtlsRoundTripper(cfg *config.SDKConfig) *utlsRoundTripper {
	...
	var proxyURL, sourceIP string
	if cfg != nil {
		proxyURL = cfg.ProxyURL
		sourceIP = cfg.SourceIP
		proxyDialer, mode, errBuild := proxyutil.BuildDialerWithSourceIP(proxyURL, sourceIP)
		if errBuild != nil {
			log.Errorf("failed to configure egress dialer for proxy %q and source IP %q: %v", proxyutil.Redact(proxyURL), strings.TrimSpace(sourceIP), errBuild)
	...
	roundTripper := &utlsRoundTripper{
		dialer:       dialer,
		sessionCache: claudeOAuthSessionCacheForEgress(proxyURL, sourceIP),
	}
