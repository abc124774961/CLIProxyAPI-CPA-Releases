...
type Server struct {
	...
	licenseManager          atomic.Pointer[licensing.Manager]
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	s.configureLicense(cfg)
	if optionState.pluginHost != nil {
		optionState.pluginHost.SetLicenseGate(s.licenseManager.Load())
	}
	...
	s.mgmt.SetLicenseManager(s.licenseManager.Load())
...
func (s *Server) Stop(ctx context.Context) error {
	...
	if manager := s.licenseManager.Load(); manager != nil {
		manager.Stop()
	}
