...
func newTestServerWithOptions(t *testing.T, opts ...ServerOption) *Server {
	...
	installTestLicense(t, server)
	return server
}

func installTestLicense(t *testing.T, server *Server) {
	t.Helper()
	...
	manager, err = licensing.InitializeWithConfig(licensing.Config{
		Provider:                    "test",
		ProductCode:                 "CPA",
		APIBaseURL:                  provider.URL,
		PublicKey:                   publicKey,
		StateDir:                    filepath.Join(t.TempDir(), "license"),
	...
	if err != nil {
		t.Fatalf("initialize test license: %v", err)
	}
	if err := manager.Activate(context.Background(), "test-code"); err != nil {
		t.Fatalf("activate test license: %v", err)
	}
	server.licenseManager.Store(manager)
