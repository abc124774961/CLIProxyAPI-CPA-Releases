...
func TestRequestAntigravityTokenRequiresRuntimeOAuthCredentials(t *testing.T) {
	t.Setenv(antigravityauth.OAuthClientIDEnv, "")
	t.Setenv(antigravityauth.OAuthClientSecretEnv, "")

	h := NewHandlerWithoutConfigFilePath(&config.Config{}, nil)
	router := gin.New()
	router.GET("/v0/management/antigravity-auth-url", h.RequestAntigravityToken)

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodGet, "/v0/management/antigravity-auth-url", nil)
	...
	if !strings.Contains(body, antigravityauth.OAuthClientIDEnv) || !strings.Contains(body, antigravityauth.OAuthClientSecretEnv) {
		t.Fatalf("response does not identify missing OAuth variables: %s", body)
	}
}
