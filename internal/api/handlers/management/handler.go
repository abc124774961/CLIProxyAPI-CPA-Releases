...
// SetLicenseManager updates the server-side license bridge used by premium
// plugin download and lifecycle endpoints. The manager keeps lease material
// private; handlers only receive verified package bytes and public status.
func (h *Handler) SetLicenseManager(manager *licensing.Manager) {
	if h == nil {
		return
	}
	h.mu.Lock()
