...
func (h *Handler) storeUploadedAuthFile(ctx context.Context, file *multipart.FileHeader, defaults authFileImportDefaults) (string, error) {
	if file == nil {
...
func (h *Handler) buildAuthFromFileData(path string, data []byte) (*coreauth.Auth, error) {
	...
	if h != nil && h.authManager != nil {
		if existing, ok := h.authManager.GetByID(authID); ok {
			...
			// A freshly synthesized auth may already carry a more up-to-date runtime
			// (for example, a completed agent-identity registration replacing a
			// pending recovery runtime). Preserve the existing runtime only when the
			// synthesized record did not install one itself.
			if auth.Runtime == nil {
